...
export const ProductDetails: React.FC<Props> = ({ product }) => {
  ...
  useEffect(() => {
    async function getProduct(productId: string) {
      ...
      const productFromData: Product = resData.find(
        (item: Product) => item.phoneId === productId,
      );
      ...
      <div className="product-details__section">
        ...
        <div className="product-details__info">
          <div className="product-details__options">
            <span className="product-details__options-title">
              Available colors
            </span>
          ...
          <div>
            ...
            <div className="product-details__buttons">
              {fetchedProduct && <AddToCartButton product={fetchedProduct} />}

              {fetchedProduct && <ButtonFovourite product={fetchedProduct} />}
            </div>
          </div>
        ...
        <div className="product-details__about" data-cy="productDescription">
          ...
          <div className="product-details__description">
            {description.map(({ title, text }) => (
              <div key={title}>
                <h3 className="product-details__paragraph-title">{title}</h3>
                <p className="product-details__paragraph">{text}</p>
              </div>
            ))}
          </div>
        </div>
