...
export const ProductCard: React.FC<Props> = ({ product }) => {
  ...
  return (
    <div className="product-card" data-cy="cardsContainer">
      <Link to={`${path}`} className="product-card__img-container">
        <img
          src={`${BASE_URL}${image}`}
      ...
      <div className="product-card__info">
        <Link to={`${phoneId}`} className="product-card__name">
          {name}
        </Link>

        <div className="product-card__prices">
          <span className="product-card__discount-price">{`$${price}`}</span>

          <span className="product-card__start-price">{`$${fullPrice}`}</span>
        </div>
      </div>
