        ...
        <Await resolve={products} errorElement={<ErrorElement />}>
          {loadedProducts => {
            const productsAmount = loadedProducts.filter(
              (product: Product) => product.category === 'tablet',
            ).length;
            ...
            return (
              <>
                <div className="tablets-page__section">
                  ...
                  <span className="tablets-page__models-amount">
                    {`${productsAmount} model${productsAmount !== 1 ? 's' : ''}`}
                  </span>
                </div>

                <div className="tablets-page__main-content">
                  {!productsAmount && (
                    <NoProducts>Tablets not found.</NoProducts>
                  )}
                </div>
              </>
            );
