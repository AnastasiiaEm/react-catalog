  ...
  return (
    <div className="accessories-page">
      ...
      <Suspense fallback={<Loader />}>
        <Await resolve={products} errorElement={<ErrorElement />}>
          {loadedProducts => {
            const productsAmount = loadedProducts.filter(
              (product: Product) => product.category === 'accessory',
            ).length;
            ...
            return (
              <>
                <div className="accessories-page__section">
                  ...
                  <span className="accessories-page__models-amount">
                    {`${productsAmount} model${productsAmount !== 1 ? 's' : ''}`}
                  </span>
                </div>

                <div className="accessories-page__main-content">
                  <div className="accessories-page__products">
                    {!productsAmount && (
                      <NoProducts>Accessories not found.</NoProducts>
                    )}
                  </div>
                </div>
              </>
